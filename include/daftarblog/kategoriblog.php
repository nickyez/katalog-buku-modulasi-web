<?php 
    ...
    // query SQL
    $sql_blog = "SELECT `b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y') as `tgl`, `b`.`isi`,`b`.`id_user`, `u`.`nama`,`b`.`id_blog` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user` = `u`.`id_user` WHERE `b`.`id_kategori_blog` = $data ORDER BY `b`.`tanggal` DESC LIMIT $posisi,$batas";
...
<nav aria-label="blog-pagination">
    <?php 
        $sql_blog = "SELECT `b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y') as `tgl`, `b`.`isi`,`b`.`id_user`, `u`.`nama`,`b`.`id_blog` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user` = `u`.`id_user` WHERE `b`.`id_kategori_blog` = $data ORDER BY `tgl`";
        $query_blog = mysqli_query($koneksi, $sql_blog);
    ...
    <?php
                                if($jum_halaman==0){
                                    ...
                                    if($halaman!=1){
                                        echo "
                                            <li class='page-item'>
                                               <a class='page-link'href='index.php?include=daftar-blog-kategori&data=$data&halaman=1'>Terbaru</a>
                                            </li>
                                            ";
                                            echo "
                                                <li class='page-item'>
                                    ...
                                    for($i=1; $i<=$jum_halaman; $i++){
                                        ...
                                        if($halaman!=$jum_halaman){
                                            ...
                                            ";
                                            echo "
                                            <li class='page-item'><a class='page-link' href='index.php?include=daftar-blog-kategori&data=$data&halaman=$jum_halaman'>Terlama</a></li>
                                            ";
                                        }
                                    }
